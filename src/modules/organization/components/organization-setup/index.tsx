      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-gray-100 flex flex-col items-center p-10 relative">
        ...
        <div className="flex items-center justify-center w-full max-w-md mb-12">
          <div className="flex flex-col items-center">
            ...
            <span className={`text-xs mt-2 font-medium ${step >= 1 ? 'text-blue-600' : 'text-gray-400'}`}>Organization</span>
          </div>
          <div className={`flex-1 h-px mx-4 ${step >= 2 ? 'bg-blue-600' : 'bg-gray-200'}`} />

          <div className="flex flex-col items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 2 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
              {step > 2 ? <Check size={16} /> : '2'}
            </div>
            <span className={`text-xs mt-2 font-medium ${step >= 2 ? 'text-blue-600' : 'text-gray-400'}`}>
          ...
          <div className="flex flex-col items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 3 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
              3
            </div>
            <span className={`text-xs mt-2 font-medium ${step >= 3 ? 'text-blue-600' : 'text-gray-400'}`}>
        ...
        <div className="w-full bg-white border border-gray-200 rounded-xl p-8">
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex justify-between items-center mt-10">
                <button onClick={onComplete} className="text-sm font-medium text-gray-500 hover:text-gray-900">
                  &lt; Back to Sign Up
                </button>
                <WpButton
                  onClick={handleNextStep}
                  disabled={!orgName.trim() || !orgSlug.trim() || !industry || !orgSize || !country || isLoading}
                  isLoading={isLoading}
                >
                  Continue
                </WpButton>
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-gray-100 text-gray-600 text-xs font-medium">Optional</span>
              </div>
              <p className="text-gray-500 mb-8">Add teammates now or invite them later from Settings.</p>
              ...
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl flex gap-3 text-sm text-blue-800 mb-8">
                <div className="shrink-0 mt-0.5">
                  <div className="w-5 h-5 rounded-full border border-blue-400 flex items-center justify-center text-xs font-serif italic">i</div>
                </div>
                <p>
                  Invitees will receive an email to join{' '}
                  <strong className="font-semibold">{orgName || 'your organization'}</strong> on WorkPilot.
                  You can manage team members any time from <strong>Settings &rarr; Members</strong>.
                </p>
              </div>
              ...
              <div className="flex justify-between items-center">
                <button onClick={handleBack} disabled={isLoading} className="text-sm font-medium text-gray-500 hover:text-gray-900 disabled:opacity-50">
                  &lt; Back
                </button>
                <div className="flex gap-3">
                  <WpButton variant="secondary" onClick={handleSkipStep} disabled={isLoading}>Skip for now</WpButton>
                  <WpButton onClick={handleNextStep} isLoading={isLoading}>Continue</WpButton>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <p className="text-gray-500 mb-8">Upload your logo. You can always update this later.</p>
