  ...
  const scheduleUpdate = useCallback(
    (
      ...
      if (targetSprintId && !isTargetSprintActive && normalizedStatus === 'in progress') {
        targetStatusId = userStoryStatuses.find(
          (status) =>
            status.name.toLowerCase().trim().replace(/_/g, ' ') === 'todo'
        )?.id;
      }
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      if (isStoryDrag) {
        ...
        const isTargetSprintActive =
          String(targetSprint?.status ?? '').toLowerCase() === 'active';
        ...
        setOptimisticUpdates((prev) => {
          ...
          next.set(storyId, {
            ...
            statusId:
              normalizedTarget && isTargetSprintActive &&
