    ...
    async (event: DragEndEvent) => {
      ...
      if (isTaskDrag) {
        ...
        if (targetType === 'story' || targetStoryId || overIdStr.startsWith('story-')) {
          ...
          // Broad invalidation so the drawer always gets fresh data regardless of key vs id
          queryClient.invalidateQueries({
            queryKey: ['user-story', effectiveProjectId],
          });
          queryClient.invalidateQueries({
            queryKey: ['user-stories', effectiveProjectId],
          });
          // Invalidate child tasks query used by ChildTasksPanel
          queryClient.invalidateQueries({
            queryKey: ['tasks', effectiveProjectId, 'child', storyId],
          });
          queryClient.invalidateQueries({
      ...
      if (isStoryDrag) {
        ...
        const isTargetSprintActive = String(targetSprint?.status ?? '').toLowerCase() === 'active';
    ...
    >
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col lg:flex-row gap-4">
          ...
          {canViewSprints && (
            <div
              className="w-full lg:w-[480px] xl:w-[560px] 2xl:w-[640px] flex-none overflow-y-auto [scrollbar-width:thin] pr-0 sm:pr-1 border-t lg:border-t-0 lg:border-l dark:border-slate-700 pt-4 lg:pt-0 lg:pl-4"
              style={{ minHeight: '400px' }}
            >
