...
export const SprintDropZone = ({
  ...
  const [isOpen, setIsOpen] = useState(String(sprint.status).toLowerCase() === 'active');
  const [isDirectTasksOpen, setIsDirectTasksOpen] = useState(true);
  const { canViewUserStories, canViewTasks, canEditSprint } = usePermissions();
  ...
  } = useGetSprintUserStories(projectId, sprint.id, !!projectId && canViewUserStories && isOpen);
  ...
  } = useGetSprintOrphanTasks(projectId, sprint.id, !!projectId && canViewTasks && isOpen);
    ...
    >
      ...
      >
        ...
        <span
          onClick={(e) => {
            e.stopPropagation();
            push(`/projects/sprints/tasks?sprintId=${sprint.id}&projectId=${projectId}`);
          }}
        ...
        <span className="text-xs font-medium px-2 py-0.5 rounded-full whitespace-nowrap shrink-0 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
          {sprint.status}
        </span>
