...
interface WorkflowTransitionsProps {
  ...
  onAddTransition: (fromStatusId: string, toStatusId: string) => void;
...
const WorkflowTransitions = ({
  ...
  const getStatus = (id: string) => statuses.find((status) => status.id === id);
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {transitions.length === 0 ? (
          <div className="flex min-h-[180px] flex-col items-center justify-center px-5 text-center">
            <div className="mb-2 text-[13px] font-medium text-gray-700">No transitions yet</div>

            <div className="text-[11px] leading-5 text-gray-400">
              Create a transition to define how work can move between statuses.
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            {transitions.map((transition) => {
              const fromStatus = getStatus(transition.fromStatusId);

              const toStatus = getStatus(transition.toStatusId);
      ...
      {showForm && (
        <div className="shrink-0 border-t border-gray-200 bg-gray-50 p-3">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-[12px] font-semibold text-gray-800">Add transition</span>
          ...
          <div className="mb-2.5">
            <label className="mb-1 block text-[10px] font-medium text-gray-500">From status</label>
          ...
          {/* To */}
          <div className="mb-2.5">
            <label className="mb-1 block text-[10px] font-medium text-gray-500">To status</label>
          ...
          {error && <div className="mb-2.5 text-[10px] text-red-500">{error}</div>}

          <div className="flex gap-2">
            <WpButton type="button" size="sm" onClick={handleCreate} className="flex-1">
              Add transition
            </WpButton>
