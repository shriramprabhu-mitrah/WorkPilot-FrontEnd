...
export default function Panel({ title, subtitle, className = '', children }: PanelProps) {
  return (
    ...
    >
      <p className="font-semibold text-sm text-gray-900 dark:text-slate-100">{title}</p>
      {subtitle && <p className="text-xs mb-2 text-gray-500 dark:text-slate-400">{subtitle}</p>}
      {children}
    </div>
  );
}
