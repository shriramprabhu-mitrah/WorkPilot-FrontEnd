...
export const TaskDetailDrawer = ({ task, onClose, onUpdate, onDelete }: TaskDetailDrawerProps) => {
  ...
  const {
    ...
    error,
    refetchTask,
  } = useGetTaskById(effectiveProjectId, effectiveTaskId);
        ...
        {isError ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50/40 dark:bg-slate-900">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-2">
              {isNotFound ? 'Task Not Found' : 'Failed to Load Task'}
            </h2>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-16 px-2.5 py-1.5 text-sm border rounded-lg dark:bg-slate-800 dark:border-slate-600 focus:outline-none focus:border-blue-500"
                          placeholder="0"
                          disabled={!canEditTask}
                        />
